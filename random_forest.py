...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        random_index = np.random.choice(range(len(XX)), size=n, replace=True)
        samples = [XX[x][:-1:] for x in random_index] # sampled dataset
        labels = [XX[x][-1] for x in random_index]  # class labels for the sampled records
    ...
    def fitting(self):
        ...
        for j in range(self.num_trees):
            t = DecisionTree()
            t.learn(self.bootstraps_datasets[j], self.bootstraps_labels[j])
            self.decision_trees.append(t)
        pass      
